<div class="card card-dashboard" data-panel-id="online_visitors" id="widget-onvisitors" ng-class="lhc.toggleWidgetData['onvisitors_widget_exp'] !== true ? 'active' : ''" ng-init="lhc.getToggleWidget('onvisitors_widget_exp')">
	<div class="card-header">
		...
		<a title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('pagelayout/pagelayout','collapse/expand')?>" ng-click="lhc.toggleWidget('onvisitors_widget_exp')" class="fs24 float-left material-icons exp-cntr">{{lhc.toggleWidgetData['onvisitors_widget_exp'] == false ? 'expand_less' : 'expand_more'}}</a>
	</div>
					...
					<tr ng-repeat="ou in group.ou | orderBy:online.predicate:online.reverse | filter:query track by ou.id" id="uo-vid-{{ou.vid}}" class="online-user-filter-row" ng-class="{<?php echo $onlineCheck?>}">
						<td nowrap="nowrap">
							<div class="btn-group" role="group" aria-label="...">
								<a href="#" class="btn btn-xs btn-outline-secondary" id="ou-face-{{ou.vid}}" <?php include(erLhcoreClassDesign::designtpl('lhchat/onlineusers/face_icon.tpl.php'));?> ng-click="online.showOnlineUserInfo(ou.id)"><i class="material-icons">info_outline</i>{{ou.lastactivity_ago}} | {{ou.nick}}&nbsp;<img ng-if="ou.user_country_code" ng-src="<?php echo erLhcoreClassDesign::design('images/flags');?>/{{ou.user_country_code}}.png" alt="{{ou.user_country_name}}" /></a><?php include(erLhcoreClassDesign::designtpl('lhchat/onlineusers/custom_online_button_multiinclude.tpl.php')); ?><span ng-click="online.previewChat(ou)" class="btn btn-xs btn-outline-success action-image" ng-show="ou.chat_id > 0" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/onlineusers','Preview chat')?>"><i class="material-icons mr-0">chat</i></span><span title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/onlineusers','Visits');?> ({{ou.total_visits}})" class="btn btn-xs btn-outline-info" ng-show="ou.total_visits > 1"><i class="material-icons">face</i>({{ou.total_visits}})</span><span class="btn btn-outline-success btn-xs" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/onlineusers','New');?>" ng-show="ou.total_visits == 1"><i class="material-icons mr-0">face</i></span>
                                <span title="{{ou.message_seen == 1 ? trans.msg_seen : trans.msg_not_seen}} | {{ou.operator_user_string}} <?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/onlineusers','has sent a message to the user');?>" class="btn btn-xs" ng-show="ou.operator_message" ng-class="ou.message_seen == 1 ? 'btn-outline-success' : 'btn-outline-danger'"><i class="material-icons mr-0">chat_bubble_outline</i></span>
                                <a href="#" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/onlineusers','Start a chat');?>" class="btn btn-xs btn-outline-secondary" ng-click="lhc.openModal('chat/sendnotice/'+ou.id);"><i class="material-icons mr-0">send</i></a>
							</div>
